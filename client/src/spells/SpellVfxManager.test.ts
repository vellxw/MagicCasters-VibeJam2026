...
describe('SpellVfxManager', () => {
  ...
  it('creates fallback projectile when runtime is not ready', () => {
    const projectiles = [{ id: 'p1', spellId: 'fireball' as const, x: 1, y: 2, z: 3 }];
    manager.syncProjectiles(projectiles);
    expect(scene.children.length).toBeGreaterThan(0);
  });

  it('creates fallback burst on confirmSpell when runtime is not ready', () => {
    manager.confirmSpell('fireball', 0, 0, 0);
    expect(scene.children.length).toBeGreaterThan(0);
  });

  it('cleans up everything on reset', () => {
    manager.syncProjectiles([{ id: 'p1', spellId: 'fireball' as const, x: 0, y: 0, z: 0 }]);
    manager.confirmSpell('fireball', 0, 0, 0);
    manager.reset();
  ...
  it('registers attach points', () => {
    ...
    expect(() => manager.playAtAttachPoint('fireball_cast', 'player1', 'caster_hand_right')).not.toThrow();
  });
